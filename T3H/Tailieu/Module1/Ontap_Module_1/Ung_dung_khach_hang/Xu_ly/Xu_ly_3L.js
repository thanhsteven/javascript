...
// API Danh sách sản phẩm -> Thêm tham số cho: api_Danh_sach_san_pham(Tham_so_url)
function api_Danh_sach_san_pham(Tham_so_url) {
    var Du_lieu = {};
    var Xu_ly_HTTP = new XMLHttpRequest();
    var Dia_chi_url = Tham_so_url;
    var Dia_chi_Xu_ly = `${Dia_chi_Dich_vu}/${Dia_chi_url}`;
    Xu_ly_HTTP.open("GET", Dia_chi_Xu_ly, false);
    Xu_ly_HTTP.send("");
    var Chuoi_JSON = Xu_ly_HTTP.responseText;
    if (Chuoi_JSON != "") {
...
// Hàm Xuất thông tin sản phẩm -> Thêm tham số cho: Xuat_Danh_sach_san_pham(Danh_sach, Th_Cha,loai_san_pham)
function Xuat_Danh_sach_san_pham(Danh_sach, Th_Cha, loai_san_pham) {
    Th_Cha.innerHTML = "";
    Danh_sach.forEach(san_pham => {
        var nhom_san_pham = `Nhom_${loai_san_pham}`;
        var The_hien = document.createElement("div")
        The_hien.className = "card m-2";
        The_hien.style.cssText = "width:17rem;float:left;";
        var Noi_dung_HTML =
            `<div class="card-img-top card-background-image"
            style="background-image: url(${Dia_chi_Dich_vu}/${san_pham.Ma_so}.png)"></div>
        <div class="card-body" style="height: 140px;">
            <p class="card-title text-primary">${san_pham.Ten}</p>
            <p class="card-text text-danger">Giá: ${Tao_Chuoi_The_hien_So_nguyen_duong(san_pham.Don_gia_Ban)} đồng </p>
            Hệ điều hành: <strong>${san_pham[`${nhom_san_pham}`].Ma_so}</strong>
        </div>`;
        ...
        Th_Thong_bao.innerHTML = `Tổng số sản phẩm <strong>${Danh_sach.length}</strong>`;
    });
};
...
function Tao_Chuoi_The_hien_So_thuc_duong(So_thuc, So_so_le) {
    ...
    if (!So_so_le)
        So_so_le = 2;
    var Thanh_phan_con = So_thuc.toFixed(So_so_le).split(".");
...
function La_Ngay_Hien_hanh(Ngay) {
    ...
    Ngay = new Date(Ngay);
    var Kq = Ngay_Hien_hanh.getDate() == Ngay.getDate() && Ngay_Hien_hanh.getMonth() == Ngay.getMonth() && Ngay_Hien_hanh.getFullYear() == Ngay.getFullYear();
    return Kq;
};
...
function Nhap_Ngay(Th_Ngay) {
    var Kq = {};
    var Chuoi_Ngay = Th_Ngay.value.trim();
